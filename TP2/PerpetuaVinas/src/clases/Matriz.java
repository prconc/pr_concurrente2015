...
public class Matriz 
{
	...
	public Matriz plus(Matriz B) {
		Matriz A = this;
		if (B.getFil() != A.getFil() || B.getCol() != A.getCol()) throw new RuntimeException("Dimensiones incompatibles");
	...
	public Matriz minus(Matriz B) {
		Matriz A = this;
		if (B.getFil() != A.getFil() || B.getCol() != A.getCol()) throw new RuntimeException("Dimensiones no compatibles.");
		Matriz C = new Matriz(getFil(), getCol());
		for (int i = 0; i < getFil(); i++)
			for (int j = 0; j < getCol(); j++)
				C.setDato(i,j,A.getDato()[i][j] - B.getDato()[i][j]);
	...
	public Matriz mult(Matriz B) {
		Matriz A = this;
		if (A.getCol() != B.getFil()) throw new RuntimeException("Dimensiones no compatibles.");
